...
## PHYS 2211 Online
## Lab 3: Black Hole - Part 1
## @file NguyenTu-PHYS2211-Lab3-P1.py
## @editor Ethan Nguyen-Tu
## @updated 5 March 2023

seed="enguyentu3@gatech.edu"
...
G = 6.7e-11 # units: m^3 * kg^-1 * s^-2 = N * m^2 * kg^-2

# Mass of the star
star.m = 2.0e30 # Given mass of the star is 2.0e30 kg

# Time interval between imported data points
deltat = 24 * 60 * 60 # seconds in a day: 24 hrs * 60 min/hr & 60 sec/min
...
idx = 1
mBH_sum = 0 # sum of the approximated black hole masses over the data values

while idx < (len(X) - 1):  # iterate over data values
    rate(100) # How fast the simulation runs: Larger # = Faster
    ...
    p_init = v_init * star.m
    p_final = v_final * star.m
    ...
    # Calculate dp/dt parallel (to phat)
    phat = p_final / mag(p_final)
    dmagpdt = (mag(p_final) - mag(p_init)) / deltat
    dpdt_par = dmagpdt * phat # definition of a vector

    # Use Newton's 2nd Law to calculate Fnet_par
    Fnet_par = dpdt_par

    # Calculate dp/dt perpendicular (to phat)
    dpdt_perp = dpdt - dpdt_par # rearragement of dpdt = dpdt_par + dpdt_perp

    # Newton's 2nd Law to calculate Fnet_perp
    Fnet_perp = dpdt_perp # Fnet = dpdet = dpdt_par + dpdt_perp

    # Calculate the mass of the black hole 
    mBH = (mag(Fnet) * mag(star.pos - origin.pos)**2) / (G * star.m)
    print(mBH)
    mBH_sum += mBH;
    ...
    arrowscale = 7e-18 # determines drawn arrow lengths that represent vectors
    arrowFnet.pos = star.pos
    arrowFnet.axis = arrowscale * Fnet
    arrowFnet_par.pos = star.pos
    arrowFnet_par.axis = arrowscale * Fnet_par
    arrowFnet_perp.pos = star.pos
    arrowFnet_perp.axis = arrowscale * Fnet_perp
    
    t = t + deltat # Advances the clock
    idx = idx + 1 # Advances the index to step through all data points
...
print("Approx. Black Hole Mass =",mBH_sum/(len(X)-2)) # averages approximations
